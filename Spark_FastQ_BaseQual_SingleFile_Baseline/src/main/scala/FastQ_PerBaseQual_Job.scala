...
object FastQ_PerBaseQual_Job {
  ...
  def main(args: Array[String]): Unit = {
    if (args.length != 2) {
      println("usage: spark-submit Spark_FastQ_BaseQual_SingleFile_Baseline-1.0.jar <fastQ input file> <output dir>")
      return;
    }
    ...
    conf.setAppName("Spark_FastQ_PerBaseQual_SingleFile_Baseline")
