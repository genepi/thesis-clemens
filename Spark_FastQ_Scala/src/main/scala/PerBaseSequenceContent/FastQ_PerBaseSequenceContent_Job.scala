...
object FastQ_PerBaseSequenceContent_Job {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //force evaluation of previously defined RDD transformations
    val tmp : Array[Pair[Int,BaseSequenceContent]] = baseSequenceContent.collect()
    val baseSequenceContentComputed = sc.parallelize(tmp)

    baseSequenceContentComputed.foreach( record => record._2.calculatePercentageOfBaseOccurrences() )

    //sort result
    val sortedRes = baseSequenceContentComputed.sortBy( record => record._1 )
