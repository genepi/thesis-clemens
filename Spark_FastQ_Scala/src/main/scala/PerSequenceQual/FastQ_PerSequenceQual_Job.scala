...
object FastQ_PerSequenceQual_Job {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val conf = new SparkConf().setAppName("Spark_FastQ_PerSequenceQual_Scala")
    ...
    //mapping step
    val qualityScores: RDD[Pair[Int,Int]] = myRdd.map( record => FastQ_PerSequenceQual_Mapper.map( record ) )
