...
object NaiveVariantCaller_Job {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // TODO temporarily commented for ease of testing
...
//    }
//
//    myRDD.map(record => record._1 + "," + record._2).saveAsTextFile(outputPath);


    // filter step
    val filteredParquetFileRDD: RDD[AlignmentRecord] = parquetFileRDD.filter( record => NaiveVariantCaller_Filter.readFullfillsRequirements(record) )



//    // TODO remove "old" attempt to handle the data
